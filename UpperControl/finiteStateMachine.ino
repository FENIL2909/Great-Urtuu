...
{
  if((state != checkArena) && (state != waitForMR2) && (state != completeManual) && (state != afterAlignGobi))
  {
    getYaw(); 
    handleXY();
  ...
  switch(state)
  {
    case mpuCalibration:
                      ...
                      getYaw();
                      if(abs(robotYaw - prevYaw) <= 1.0 && millis() - calibrationTime > 3000)
                      {
                        mpuConst = robotYaw;
                        state = checkArena;
                        whiteLEDon;
    ...
    case startSignal:
                      ...
                      {
                        state = khangaiUrtuu;
                        if(!digitalRead(resetAlignButton))
                          {
                            state = reachShagai1;
    ...
    case alignBridge:
                      blinkLED(colorr);                        
                      fsmFlag = traceLine(alignX, alignY, shiftX, arenaFlag*shiftY,speeed,ALL_QUAD,STOP,10);
    ...
    case reachShagai1:
                     ...
                     fsmFlag = traceLinePID(0, 0, 68, -arenaFlag*60 , speeed, ALL_QUAD, STOP, 5, 15);//,0);
                     if(fsmFlag)
                     {
                       state = reachShagai1;
