...
bool Curve :: traceForest(float vel)
{
   ...
   {
    if(arenaFlag == RED)
      flag = traceLinePID(xaRED[i-1]+shiftX,arenaFlag*yaRED[i-1]+arenaFlag*shiftY,xaRED[i]+shiftX,arenaFlag*yaRED[i]+arenaFlag*shiftY,vel,ALL_QUAD,CONTINUE,25,20);//,0);
    else
      flag = traceLinePID(xaBLUE[i-1]+shiftX,arenaFlag*yaBLUE[i-1]+arenaFlag*shiftY,xaBLUE[i]+shiftX,arenaFlag*yaBLUE[i]+arenaFlag*shiftY,vel,ALL_QUAD,CONTINUE,25,20);//,0);
   }
   else 
   {  
     i++;
